...
M = 128 # taken from the default IndexHnswArguments for the RAG example on hugging face referenced by the RAG paper
matching_pattern = "?"
vector_length = 1536
# efSearch and efConstruction are kept at default like in RAG
...
con.sql(f"CALL FAISS_CREATE('flat', {vector_length}, 'IVF{clusters}_HNSW{M},Flat')") # the index is based on https://github.com/facebookresearch/faiss/wiki/Guidelines-to-choose-an-index, quite important to use low memory and 10M-100M vectors. But PQ16 wasnt added because it requires training
